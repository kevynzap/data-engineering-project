...
@dag(
    dag_id = "products_to_bronze",
    start_date = datetime(2026, 1, 1),
    schedule = None,
    catchup = False,
    tags = ["products", "extract", "bronze"]
)
...
def main ():
    ...
    wait_for_file = FileSensor(
        ...
        filepath="input_data/products/*.csv", 
        poke_interval=30,
        timeout=60 * 5,
    ...
    run_spark = SparkSubmitOperator(
        ...
        application="/opt/spark_jobs/products_to_bronze.py",
        conf=config,
        verbose=True
    )

    products_silver = TriggerDagRunOperator(
        task_id="trigger_products_to_silver",
        trigger_dag_id="products_to_silver"
    )

    wait_for_file >> run_spark >> products_silver
