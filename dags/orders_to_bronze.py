...
def main ():
    ...
    wait_for_file = FileSensor(
        ...
        filepath="input_data/orders/*.csv", 
        poke_interval=30,
        timeout=60 * 5,
    ...
    run_spark = SparkSubmitOperator(
        ...
        application="/opt/spark_jobs/orders_to_bronze.py",
        conf=config,
        verbose=True
    )

    orders_silver = TriggerDagRunOperator(
        task_id="trigger_orders_to_silver",
        trigger_dag_id="orders_to_silver"
    )

    wait_for_file >> run_spark >> orders_silver
